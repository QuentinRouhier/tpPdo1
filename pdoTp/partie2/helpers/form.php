...
class form {
     ...
     * @return boolean
     */
    static function checkMartialStatut($martialStatut) {
        if ($martialStatut == 0) {
            // tu ajoute dans l'attribue birthdate le contenu de ce que l'utilisateur a envoyer
            $MartialStatut = TRUE;
        } else {
    ...
    /**
     * Check si la regex match ou non 
     * @param type $organisme prend ce qu'il y a en paramettre dans le controlleur
     * @return boolean
     */
    static function checkOrganisme($organisme) {
        //regex pour l'e nom et le prenom'organisme
        $regexOrganisme = '/^([a-z0-9àéèëêù\'ïîâäöôç\-, ]){2,50}$/i';
        // si la regex n'est pas en accord avec ce que l'utilisateur a envoyer
        if (preg_match($regexOrganisme, $organisme)) {
            // tu ajoute dans l'attribue birthdate le contenu de ce que l'utilisateur a envoyer
            $correctOrganisme = TRUE;
        } else {
            $correctOrganisme = FALSE;
        }
        return $correctOrganisme;
    }

    static function checkAmount($amount) {
        //regex pour les montant
        $regexAmount = '/^([1-9]){1}([0-9]){1,6}/';
        // si la regex n'est pas en accord avec ce que l'utilisateur a envoyer
        if (preg_match($regexAmount, $amount)) {
            // tu ajoute dans l'attribue birthdate le contenu de ce que l'utilisateur a envoyer
            $correctOrganisme = TRUE;
        } else {
            $correctOrganisme = FALSE;
